...
function openInfoModal() {
    const columnInfo = [
        { label: 'Symbol', desc: 'Ticker symbol' },
        { label: 'Price', desc: 'Current stock price' },
        { label: 'Dividend Yield', desc: 'Annual dividends as a percent of price' },
        { label: 'P/E Ratio', desc: 'Price to earnings ratio' },
        {
            label: 'ROCE',
            desc: '-Return on Capital Employed\n-ROCE above 15% may indicate better profitability'
        },
        {
            label: 'Interest Coverage',
            desc: '-Measures the ability to cover interest\n-A ratio of 10x suggests a more controlled financial position'
        },
        {
            label: 'Gross Margin',
            desc: '-Gross profit as a percent of revenue\n-A margin above 30% may indicate a stronger competitive postion'
        },
        {
            label: 'Net Margin',
            desc: '-Portion of revenue that exceeds all expenses\n-A net margin above 15% suggests a greater ability of generating net income'
        },
        {
            label: 'Cash Conversion Ratio (FCF)',
            desc: '-How efficiently a company is converting its income into free cash flo w\n-A higher FCF can suggest dividend increases, buyback programs'
        },
        {
            label: 'Gross Profit / Assets',
            desc: '-Gross profit relative to total assets\n-Shows how efficiently a company uses its assets to generate profit'
        },
        { label: 'Score', desc: 'Composite financial score 0-100' },
        { label: 'AI Analysis', desc: 'Qualitative insights generated by AI' }
    ];

    let list = '<ul class="info-list">';
    columnInfo.forEach(item => {
        const formatted = item.desc.replace(/\n/g, '<br>');
        list += `<li><strong>${item.label}</strong>: ${formatted}</li>`;
    });
    list += '</ul>';
